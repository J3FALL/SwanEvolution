import copy, Data, datetime, math, Log, Supply
import numpy as np
import Consts
...
def getCoevError(couples, station, point, station_obs, point_obs):
    ...
    print "Current time when coefficients are computed :  {0}".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def getBestIndividuals(individuals, anotherIndividuals, modelDate):
    ...
    for stationId in range(0, Consts.Models.Observations.Stations.FullCount):
        res = getCoevError(couples,  Consts.Models.SWAN.Stations.FullNames[stationId], Consts.Models.SWAN.OutputColumns.Hsig,
    ...
    front, MSError = selection(modelDate, map(lambda c, errors: Coindividual(Supply.ModelDate.numOfPopulation+1, c, errors, -1), couples, errorAtStations))
...
def getRawFitness(coindividuals, front, index):
    ...
    if index > len(front) - 1 or index < 0:
        ...
        return 1
    return 1 + len(filter(lambda ind: front[index].errors[0] <= ind.errors[0] and front[index].errors[1] <= ind.errors[1], copy.deepcopy(coindividuals))) / len(coindividuals)
...
def getErrors(couples, station, point, station_obs, point_obs):
    ...
    sumVal = [np.sqrt(sum(map(lambda obs, est1, est2: (obs - (est1*couple[0].ensCoeff+est2*couple[1].ensCoeff)) ** 2, obs[:time_range], couple[0].forecasts[stationIndex][:time_range],couple[1].forecasts[stationIndex][:time_range])) / len(couples[0][0].forecasts[stationIndex][:time_range])) for couple in couples]
    return sumVal
...
def truncateFront(front, coindividuals): #returns front which size is minimum from 1 to 4 and maximum sqrt(len(front))
    ...
    while kNearest + 2 < len(front): # +2 as it is a distance between any two objects
        ...
        if index + 1 == len(distances):
            del front[index]
            continue
        if getRawFitness(coindividuals, front, index) * distances[index - 1] > getRawFitness(coindividuals, front, index + 1) * distances[index + 1]:
            del front[index + 1]
        else:
            del front[index]
    return front
...
def preprocess(coindividuals):
    ...
    for i in range(0,7):
        individuals1.append(Supply.runModel([Consts.Models.SWAN.Parameters.WindCoeff.first[i],1,Consts.Models.SWAN.Parameters.WhiteCappingRate.first[i],Consts.Models.SWAN.Parameters.WaveSteepness.defaultValue,Consts.Models.SWAN.Parameters.ensCoeff.first[i]],modelDate))
        individuals1[i].ensCoeff=Consts.Models.SWAN.Parameters.ensCoeff.first[i]
    Consts.State.forcingId=1
    for i in range(0,7):
         individuals2.append(Supply.runModel([Consts.Models.SWAN.Parameters.WindCoeff.first2[i],1,Consts.Models.SWAN.Parameters.WhiteCappingRate.first2[i],Consts.Models.SWAN.Parameters.WaveSteepness.defaultValue,Consts.Models.SWAN.Parameters.ensCoeff.first[i]],modelDate))
