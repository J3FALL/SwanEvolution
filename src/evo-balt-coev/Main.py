...
def crossover(individuals,
    ...
    for i in range(Consts.Evolution.populationMaxSize):
            if (realI>=len(individuals)):
                realI = 0
            paramsList.append((copy.copy(individuals[realI].dragFunc),copy.copy(individuals[realI].physicsType),copy.copy(individuals[realI].wcr),copy.copy(individuals[realI].ws),copy.copy(individuals[realI].ensCoeff)))
            realI +=1
...
def mutation(individuals, convergenceExtent,forced=False):  # write variable influenced by convergence extent, the higher is extent the the higher mutation's coefficient is
    ...
    for param in paramsList:
        drf = param[Consts.Models.SWAN.Parameters.WindCoeff.indInParamsArray]
        drf += sign(rnd.random()) * 0.55 * rnd.random() * paramsList[rnd.randint(0, len(paramsList) - 1)][Consts.Models.SWAN.Parameters.WindCoeff.indInParamsArray] * add
        drf = math.fabs(drf)
        ...
        if (physicsType == Consts.Models.SWAN.Parameters.PhysicsType.typesNames.index("GEN3")):  # only for GEN3
            ...
            wcr += sign(rnd.random()) * 0.75 * rnd.random() * paramsList[rnd.randint(0, len(paramsList) - 1)][Consts.Models.SWAN.Parameters.WhiteCappingRate.indInParamsArray] * add
            wcr = math.fabs(wcr)
...
def evolution(individuals1, individuals2, modelDate, convErrorOld, isConvergence=False, isCrossover=True,
    ...
    while not (isConvergence):
        ...
        individuals1 = [ind for ind in individuals1 if ind.ID in [coind.couple[0].ID for coind in [coind for coind in newCoindividuals ]]]
        individuals2 = [ind for ind in individuals2 if ind.ID in [coind.couple[1].ID for coind in [coind for coind in newCoindividuals ]]]
        ...
        if ((abs(convErrorNew/convErrorOld)>1.5 and Supply.ModelDate.numOfPopulation >= Consts.Evolution.minPopulationsNumber) or Supply.ModelDate.numOfPopulation>=Consts.Evolution.maxPopulationsNumber):
            isConvergence = True
