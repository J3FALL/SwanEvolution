import Supply, Data, copy, datetime, Log, shutil, os, re
import Consts
...
def copy(new_name, name_postfix, path = Consts.Models.SWAN.pathToStatesRoot):
    ...
    os.rename(path + '\\STATES\\' + str(name) + str(name_postfix), path + '\\STATES\\' + str(new_name) + str(name_postfix))
    return 0
    ...
    if (len(list_of_state_files) > 0):
        for i, state_file in enumerate(list_of_state_files):
            ...
            for j, line in enumerate(state_file):
                list_of_state_files[0][j] = map(lambda state_0_j, state_i_j: state_0_j + state_i_j, list_of_state_files[0][j], line)
