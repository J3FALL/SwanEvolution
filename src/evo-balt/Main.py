import Supply, Data, WindData, CoevolutionData, copy, math, random as rnd, datetime, pandas as pd, Log, time, States, os
import numpy as np
...
def crossover(individuals,
    ...
    for i in range(Consts.Evolution.populationMaxSize):
            if (realI>=len(individuals)):
                realI = 0
            paramsList.append((copy.copy(individuals[realI].dragFunc),copy.copy(individuals[realI].physicsType),copy.copy(individuals[realI].wcr),copy.copy(individuals[realI].ws)))
            realI +=1
...
def mutation(individuals, convergenceExtent,forced=False):  # write variable influenced by convergence extent, the higher is extent the the higher mutation's coefficient is
    Log.write('Mutation')
    sign = lambda x: -1 if x < 0.5 else 1
    paramsList = []
    ...
    for param in paramsList:
        drf = param[Consts.Models.SWAN.Parameters.WindCoeff.indInParamsArray]
        drf += sign(rnd.random()) * 0.15 * rnd.random() * paramsList[rnd.randint(0, len(individuals) - 1)][Consts.Models.SWAN.Parameters.WindCoeff.indInParamsArray] * add
        drf = math.fabs(drf)
        ...
        if (physicsType == Consts.Models.SWAN.Parameters.PhysicsType.typesNames.index("GEN3")):  # only for GEN3
            ...
            wcr += sign(rnd.random()) * 0.25 * rnd.random() * paramsList[rnd.randint(0, len(individuals) - 1)][Consts.Models.SWAN.Parameters.WhiteCappingRate.indInParamsArray] * add
            wcr = math.fabs(wcr)
...
def evolution(individuals, modelDate, convErrorOld, isConvergence=False, isCrossover=True,
    ...
    while not (isConvergence):
        ...
        if ((abs(convErrorNew/convErrorOld)>1.2 and Supply.ModelDate.numOfPopulation >= Consts.Evolution.minPopulationsNumber) or Supply.ModelDate.numOfPopulation>=Consts.Evolution.maxPopulationsNumber):
            isConvergence = True
...
def getDominatingSolutions(individuals, front,
    ...
    if index > len(front) - 1 or index < 0:
        print 'Index Exception, index = {0}, len of individuals = {1}'.format(index, len(front))
        return 1
...
def initializing(modelDate, bestIndividuals, individuals):  # running best of previos
    ...
    res = []
    if len(bestIndividuals) == 0:  # for better diversity add extra solutions to the mate pool at the first launch  --->  it won't happen in this version as len is specified as 5
...
def preprocess(individuals):
   ...
   # individuals.append(runModel(modelDate))
    ...
    if (Consts.State.readInitialFromFile):
        ...
        for i in range(0,7):
            individuals.append(runModel([Consts.Models.SWAN.Parameters.WindCoeff.first[i],1,Consts.Models.SWAN.Parameters.WhiteCappingRate.first[i],Consts.Models.SWAN.Parameters.WaveSteepness.defaultValue],modelDate))
...
def  runModel(params, modelDate):
    ...
    SwanFunctions.writeSwanConfig(Consts.Models.SWAN.pathToConfig(), "COMPUTE",
                                  [2,5],
                                  [Consts.Models.Observations.timePeriodsStartTimes[Consts.State.currentPeriodId],Consts.Models.Observations.timePeriodEndTimes[Consts.State.currentPeriodId]])
    ...
    SwanFunctions.writeSwanConfig(Consts.Models.SWAN.pathToConfig(), Consts.Models.SWAN.Parameters.WindCoeff.name,
                                  np.asarray([Consts.Models.SWAN.Parameters.WindCoeff.valueColumnId]), str(params[Consts.Models.SWAN.Parameters.WindCoeff.indInParamsArray]))

    #GEN
    physicsTypeName=Consts.Models.SWAN.Parameters.PhysicsType.typesNames[(params[Consts.Models.SWAN.Parameters.PhysicsType.indInParamsArray])]
    SwanFunctions.writeSwanConfig(Consts.Models.SWAN.pathToConfig(), Consts.Models.SWAN.Parameters.PhysicsType.name,
                                  np.asarray([Consts.Models.SWAN.Parameters.PhysicsType.valueColumnId]), physicsTypeName)
    #wcr
    SwanFunctions.writeSwanConfig(Consts.Models.SWAN.pathToConfig(), Consts.Models.SWAN.Parameters.WhiteCappingRate.name,
                                  np.asarray([Consts.Models.SWAN.Parameters.WhiteCappingRate.valueColumnId]),
                                  Consts.Models.SWAN.Parameters.WhiteCappingRate.name+str(params[Consts.Models.SWAN.Parameters.WhiteCappingRate.indInParamsArray]))
    #ws
    SwanFunctions.writeSwanConfig(Consts.Models.SWAN.pathToConfig(), Consts.Models.SWAN.Parameters.WaveSteepness.name,
                                  ...
                                  Consts.Models.SWAN.Parameters.WaveSteepness.name +str(Consts.Models.SWAN.Parameters.WaveSteepness.defaultValue))#str(params[Consts.Models.SWAN.Parameters.WaveSteepness.indInParamsArray]))
    ...
    for stationId in range(0, Consts.Models.Observations.Stations.FullCount):
        observationsAtStations[stationId], forecastAtStations[stationId], errorAtStations[stationId] = calculateErrors(Consts.Models.SWAN.Stations.FullNames[stationId], Consts.Models.SWAN.OutputColumns.Hsig,
    ...
    individuals = [
        Consts.individual(Supply.ModelDate.numOfIndividuals, Supply.ModelDate.numOfPopulation, selectedErrors, errorAtStations, params, forecastAtStations)]
        ...
        for forcingId in range(0, len(Consts.State.forcingType)):
            ...
            for periodId in range(0, (Consts.Models.Observations.timePeriodsCount)):
                ...
                for stationId in range (0,Consts.Models.Observations.Stations.FullCount):
                    ...
                    Consts.State.expId = "sepExp_time"+str(periodId)+"_"+str(Consts.State.forcingType[forcingId])+"_"+str(stationId+1)
                    individuals = []
...
def truncateFront(front, individuals):  # returns fron hich size is minimum from 1 to 4 and maximum sqrt(len(front))
    ...
    while kNearest + 1 < len(front):  # +3 as it is a distance between any two objects
        ...
        if index + 1 == len(distances):
            del front[index]
            continue
        if getDominatingSolutions(individuals, front, index) * distances[index - 1] > getDominatingSolutions(individuals, front,
